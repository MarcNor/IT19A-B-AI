...
data_df = pd.read_csv('../../doc2vec_dbow_2d.csv', sep=",", usecols=[0,1,2])
vectors_arr = data_df[data_df.columns[1:]].values

# Compute DBSCAN
db = DBSCAN(eps=1.5).fit(vectors_arr)
...
#prepare: read vector data
data_df = pd.read_csv('../../doc2vec_dm_2d.csv', sep=",", usecols=[0,1,2])
vectors_arr = data_df[data_df.columns[1:]].values

# Compute DBSCAN
db = DBSCAN(eps=5).fit(vectors_arr)
core_samples_mask = np.zeros_like(db.labels_, dtype=bool)
core_samples_mask[db.core_sample_indices_] = True
labels = db.labels_
